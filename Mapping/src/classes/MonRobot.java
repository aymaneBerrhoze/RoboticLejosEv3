...
public class MonRobot implements MonRobotInterface {
	...
	public void allerVers(But but) {
		...
		navigateur.goTo(point.getX() + 15*(float)Math.cos(teta + Math.PI), point.getY() - 15*(float)Math.sin(teta + Math.PI));
//		navigateur.goTo(point.getX(), point.getY() -15);
		...
		pilot.stop();
		insisterPrendreBut();
		// on verifie en meme temps que la distance est sup a 6 cm
	...
	public void insisterPoserObjet(Maison maison){
		...
		navigateur.rotateTo(navigateur.getPoseProvider().getPose().getHeading()-17);
		
	}

	public void insisterCouleurMaison(){
		float angleInitiale = navigateur.getPoseProvider().getPose().getHeading();
		int i=0;
		for(i =(int) angleInitiale; i<360 + angleInitiale; i+=10){
			if(csMaison.getColorID() == 0 || csMaison.getColorID() == 1 || csMaison.getColorID() == 3)break;
			else navigateur.rotateTo(i);
		}
		navigateur.rotateTo(i-18);
	}
	
	public void insisterPrendreBut(){
		float angleInitiale = navigateur.getPoseProvider().getPose().getHeading();
		float[] sample = new float[1];
		SampleProvider sp = us.getDistanceMode();
		for(int i =(int) angleInitiale; i<360 + angleInitiale; i+=5){
			sp.fetchSample(sample, 0);
			if(sample[0]< 0.15)break;
			else navigateur.rotateTo(i);
		}
	}
	
	public void allerA00(){
		navigateur.goTo(0, 0);
		Delay.msDelay(2000);
		while (pilot.isMoving());
		Delay.msDelay(2000);
